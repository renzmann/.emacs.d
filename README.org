...
code, and so a change of medium felt prudent.
...
Usually not quite up-to-date with the master branch, but still one version
number ahead of the most recent official release on the [[https://ftp.gnu.org/gnu/emacs/windows/][main FTP]].
...
Some security updates from Apple can cause issues with native compilation
(notably 15.4.1).  In that case, this is how I compile on mac.
...
=sudo apt-get install emacs=, is out of date.  So, typically, I'll get a release
tarball from the FTP and compile it myself.  On Debian, that requires an
=apt-get= install of the following dev libraries:
...
system-wide locations like =/usr= or =/usr/local=, so I'll just opt for a user
install under =~/.local=.
...
independent of Emacs.  Namely, =find=, =xargs=, =gcc=, and =rg=.  Even with all
...
=find= and =grep=. For instance, if we were to set the =shell-file-name= like
this:
...
Hats off to [[https://protesilaos.com/][Prot]] for his wonderful themes.
...
-Q~), I would still set most of these by hand at one point or another.
...
I'm always putting Gnu utilities under =Program Files (x86)\GnuWin32= on my
Windows machines.
...
** Keep some executables in this config directory
...
Emacs 29 introduces many features that help keep the minibuffer and completions
in tandem.
...
** Completion previews
Emacs 30 introduced in-line completion previews, which are separate from your
usual drop-down menu style of preview.
...
** Vertico
Still the GOAT for fast minibuffer completion.
...
set that up somewhere common, like =~/.local/bin/bq-shell=, and =chmod +x= on
it.
...
** Automatically install and use tree-sitter modes
#+begin_src emacs-lisp
...
* Wishlist
+ Tramp is hanging, but not when using =emacs -q=.  Prune any docker stuff from
  my config
...
+ A good tmux-like workflow for running multiple async compilation jobs, with an
  ...
  - Best way to do this is to use =C-x p M-&=, or =C-x p c= to start an async or
    compilation job, then rename the buffer using =C-x x r=.  When visiting this
    buffer, =g= will kill and restart the job.
...
+ Eshell commands for creating/managing virtualenvs
  ...
  - I just can't find a good workflow on eshell.  The =M-!= and =M-&= do
    everything I would do from eshell, but also track my environment better.
+ How do we use local emacs as EDITOR for tramp connections?  Things like
  default git messages from merges are difficult
  - This just requires a workflow change.  Do it from a terminal or ensure we're
    always using VC mode.
* Footer
