...
tarball from the FTP and compile it myself.  I typically run into trouble
(different every time) when installing to system-wide locations like =/usr= or
=/usr/local=, so I'll just opt for a user install under =~/.local=.
