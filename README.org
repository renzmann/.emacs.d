...
#+RESULTS:
| (lambda nil (progn (add-hook 'after-save-hook #'org-babel-tangle :append :local) (add-hook 'org-babel-after-execute-hook #'display-ansi-colors))) | visual-line-mode | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes |

Python has its own =:async yes= header argument we can use, so there's no need to inclkude it with ~ob-async~.

#+begin_src emacs-lisp
(setq ob-async-no-async-languages-alist '("python"))
#+end_src

I'm having trouble remmbering why I added this following line, except that I belive it has something to do with exporting to HTML with syntax highlighting

#+begin_src emacs-lisp
(setq org-html-htmlize-output-type 'css)
#+end_src

~org-mode~ provides =org-babel-tangle-jump-to-org=, which allows us to jump back to an Org source file from within the tangled code.  I needed a function that does the opposite - given the Org source file and point inside a ~src~ block, jump to the location of the tangled code.  Provided by a helpful [[https://emacs.stackexchange.com/a/69591][stackoverflow answer.]]
...
Outside the typical TODO/DONE states, I like to use DEAD as an indicator that something is fully blocked, but not done.
