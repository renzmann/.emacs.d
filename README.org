...
[[https://archive.casouri.cc/note/2023/tree-sitter-in-emacs-29/index.html][recommended workaround]] is to derive a mode that picks between them.
...
To install a grammar (e.g. for Python), I just need to do =M-x
treesit-install-language-grammar RET python= after setting
...
What I need to play with now is ~auto-mode-alist~ and ~interpreter-mode-alist~,
along with the fallback macro above to boot up the treesitter version
automatically if everything is installed, otherwise falling back to the original
mode.

Finally, I like to program "in Skittles":
