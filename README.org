...
There are a few things I set up independent of Emacs.  Namely, =find=, =xargs=, and
=rg=.  These days, I can usually install these things with =winget=:
...
1. leuven-dark is mostly grey/blue, but the green-purple in buffers is a little
   clashing.  The way this theme treats Org files is superior to deeper-blue, though.
2. modus-vivendi is probably the best theme, but requires the most tweaks to get
   comfortable.  Eventually I'd like a configured version of this theme to
   become my daily driver.
3. deeper-blue doesn't have org src highlighting by default, but I think this
   scheme is most legible in programming modes.
4. tsdh-dark not terrible, but the red outline around the minibuffer is kinda
   ugly.  Green in-buffer isn't great.
5. wombat is /okay/, but the green is not super clear in e.g. HTML mode.
   Underlining is used too frequently and looks bad.
6. NO tango-dark (lose syntax on highlighted line)
