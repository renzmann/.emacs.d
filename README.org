...
I used to bind this before I knew about =M-SPC= (~cycle-spacing~):
...
+ A prettier eshell, with colors from git
  - Setting TERM correctly, and getting rid of "not fully functional" warnings
+ Think about efficient editing motions with a cheatsheet that translates from
  vim
+ Enable proxy for a single Eshell instance
